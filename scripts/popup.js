document.addEventListener('DOMContentLoaded', function () {
  ...
  function checkURL(url) {
    ...
    const projectIssuePageRegex =
      /(https:\/\/www.drupal.org\/project\/)\w+(\/issues\/)\d+/gm;
  ...
  function readIssueContent() {
    chrome.tabs.executeScript(
      ...
      ([result] = []) => {
        // Hide 'please wait' message
        const pageStatusElement = document.querySelector(
          '.reading-page-status',
        );
        pageStatusElement.classList.add('hidden');
    ...
    function inContent(params) {
      ...
      const issueFork = document.querySelector('.fork-link')
        ? document.querySelector('.fork-link').innerText
        : false;
      const allBranches = document.querySelector('.branches')
        ? document.querySelector('.branches').children
        : false;
      ...
      const moduleVersion = document
        .querySelector('.field-name-field-issue-version')
        .children[1].innerText.replace('-dev', '');
      const loggedIn = document.querySelector('.person') ? true : false;
      const pushAccess = document.querySelector('.drupalorg-issue-fork-access')
        ? true
        : false;
  ...
  function openDevEnv() {
    ...
    const projectName =
      'DP_PROJECT_NAME=' + document.getElementById('project-name').innerText;
    const issueFork =
      'DP_ISSUE_FORK=' +
      (document.getElementById('issue-fork').innerText === 'false'
        ? ''
        : document.getElementById('issue-fork').innerText);
    const issueBranch =
      'DP_ISSUE_BRANCH=' + encodeURIComponent(getSelectValue('issue-branch'));
    const projectType =
      'DP_PROJECT_TYPE=' + document.getElementById('project-type').innerText;
    const moduleVersion =
      'DP_MODULE_VERSION=' +
      document.getElementById('module-version').innerText;
    const coreVersion = 'DP_CORE_VERSION=' + getSelectValue('core-version');
    const patchFile =
      'DP_PATCH_FILE=' +
      encodeURIComponent(getSelectValue('available-patches'));
    const installProfile =
      'DP_INSTALL_PROFILE=' +
      (getSelectValue('install-profile') === '(none)'
        ? "''"
        : getSelectValue('install-profile'));
...
async function getProjectType(projectName) {
  const url =
    'https://www.drupal.org/api-d7/node.json?field_project_machine_name=' +
    projectName;
  let obj = null;
...
async function populateDrupalCoreVersion() {
